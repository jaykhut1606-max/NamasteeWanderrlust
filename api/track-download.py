...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            trip = body.get("trip", "Munnar")

            if not email:
                return self._json(400, {"error": "Email is required"})

            # Best-effort database write — the DB being down must not block anything
            stored = False
            try:
                supabase_rpc("track_download", {
                    "user_email": email,
                    "user_name": name,
                    "trip": trip
                })
            ...
            notified = send_enquiry_alert(
                f"New itinerary enquiry — {trip} ({email})",
                build_enquiry_email("Itinerary download", email, name, trip),
            )
